...
// testInfiniteLoop
// NOTE: This test now includes an additional section that fills a gap between
// sections 5 and 6. The gap (4.31 units) is less than 2*offset (8 units),
// indicating a buffer hole caused by close segment approaches, which is now
// correctly filled by the gap-filling algorithm.
template<>
template<>
void object::test<37> ()
{
...
// testGapFillCloseSegments
// Test case for gap-filling when input segments are closer than 2*offset
// Bug report: segments (0,0)-(10,0) and (5,5)-(5,3) are 3 units apart (< 2*1.75)
// This creates a buffer hole, causing a gap in the offset curve that must be filled
template<>
template<>
void object::test<45> ()
{
    checkOffsetCurve(
        "LINESTRING (0 0, 10 0, 10 5, 5 5, 5 3)", 1.75,
        "MULTILINESTRING ((0 1.75, 3.78 1.75), (3.78 1.75, 6.22 1.75), (6.22 1.75, 8.25 1.75, 8.25 3.25, 6.75 3.25, 6.75 3))"
    );
}
